...
# Fit finish ~ speed + start (z-scored within race); return the start coefficient.
.gts_fit_beta <- function(d) {
  if (nrow(d) < 40) return(NA_real_)
  z <- function(x) { s <- stats::sd(x, na.rm = TRUE); if (is.na(s) || s == 0) return(rep(0, length(x))); (x - mean(x, na.rm = TRUE)) / s }
  dd <- d %>% group_by(race_id) %>%
    mutate(fz = z(ps), sz = z(spd), tz = z(start_ps)) %>% ungroup()
  dd <- dd[stats::complete.cases(dd[, c("fz", "sz", "tz")]), , drop = FALSE]
  if (nrow(dd) < 40) return(NA_real_)
  fit <- try(stats::lm(fz ~ sz + tz, data = dd), silent = TRUE)
  if (inherits(fit, "try-error")) return(NA_real_)
  unname(stats::coef(fit)["tz"])
}
...
gts_auto_inputs <- function(hist, entry, tgt) {
  ...
  # Residual spread of the speed->finish map, at this track if possible.
  ...
  use <- if (nrow(trk) >= 60) trk else if (nrow(typ) >= 100) typ else d
  sd_res <- stats::sd(use$fin_p - use$spd, na.rm = TRUE)
  if (!is.finite(sd_res) || sd_res <= 0) sd_res <- 0.15
  
  # Start-position coefficient: fit at track, shrink toward track type.
  b_typ <- .gts_fit_beta(typ); b_all <- .gts_fit_beta(d)
  prior_b <- if (!is.na(b_typ)) b_typ else if (!is.na(b_all)) b_all else 0
  b_trk <- .gts_fit_beta(trk)
  beta <- if (is.na(b_trk)) prior_b else {
    w <- nrow(trk) / (nrow(trk) + .gts_K_BETA); w * b_trk + (1 - w) * prior_b
  }
  ...
  # ---- DKMax: start-slot ceiling, capped by driver/team historical best -----
  ...
  slot_tab <- function(x) {
    ...
    sb <- cut(x$start_ps, brk)
    v <- tapply(x$DKSP, sb, function(z) stats::quantile(z, 0.90, na.rm = TRUE))
    ...
    for (i in seq_along(out)) if (is.na(out[i]) && i > 1) out[i] <- out[i - 1]
    for (i in rev(seq_along(out))) if (is.na(out[i]) && i < length(out)) out[i] <- out[i + 1]
    out[is.na(out)] <- 0
    out
  }
  st_trk <- slot_tab(hs[hs$track_name == tgt$track_name, , drop = FALSE])
  st_typ <- slot_tab(hs[hs$track_type == tgt$track_type, , drop = FALSE])
  base_tab <- if (!is.null(st_typ)) st_typ else slot_tab(hs)
  ceil_tab <- base_tab
  ...
  slot_src <- if (has_start) starts else rank(spd_est, ties.method = "first")
  slot_src[is.na(slot_src) | slot_src <= 0] <- F
  sb <- cut(slot_src, brk)
  slot_ceiling <- if (is.null(ceil_tab)) rep(NA_real_, F) else as.numeric(ceil_tab[as.character(sb)])
  ...
  dkmax <- round(pmin(hist_best, slot_ceiling * 2.2))
  dkmax[!is.finite(dkmax)] <- 0
...
server <- function(input, output, session) {
  ...
  output$download_input_file <- downloadHandler(
    ...
    content     = function(file) {
      ...
      # Auto-filled starting point for the driver-input columns. Derived from
      # this series' history; the user reviews and adjusts before running sims.
      auto <- NULL
      try({
        tgt_row <- values$races_sheet %>%
          filter(race_id == suppressWarnings(as.numeric(input$analysis_race_id))) %>%
          slice(1)
        hist_rows <- values$nascar_data %>%
          filter(series_id == suppressWarnings(as.numeric(input$analysis_series)),
                 race_id  != suppressWarnings(as.numeric(input$analysis_race_id)))
        if (nrow(tgt_row) == 1 && nrow(hist_rows) > 100) {
          ent <- driver_sheet %>%
            transmute(Name = Name, Car = car, Team = team,
                      DK_Salary = DKSalary, Start = Starting)
          auto <- gts_auto_inputs(hist_rows, ent, tgt_row[1, ])
        }
      }, silent = TRUE)
      
      if (!is.null(auto) && nrow(auto) == nrow(driver_sheet)) {
        idx <- match(driver_sheet$Name, auto$Name)
        for (cl in c("W", "T3", "T5", "T10", "T15", "T20", "T25", "T30", "DKMax"))
          driver_sheet[[cl]] <- auto[[cl]][idx]
      }

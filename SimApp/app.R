...
server <- function(input, output, session) {
  ...
  observeEvent(input$run_dk_optimization, {
    ...
    tryCatch({
      ...
      if (rv$sport == "GOLF") {
        ...
        # element is NULL, so the vector is one short and the ownership step
        # dies with "non-conformable arrays". The FD path below already passes
        # mvp_multiplier; this one was missing its counterpart.
        opt_config <- list(roster_size=rv$config$roster_sizes$DK, salary_cap=rv$config$salary_caps$DK,
                           percentiles=c(0.01,0.05,0.10,0.20), platform_col="DKScore",
  ...
  make_range_sliders <- function(lp) {
    renderUI({
      ...
      # No sliders at all is a real case, not an error: on a preseason slate
      # every numeric column here is a constant (one salary, zero ownership), so
      # each one is skipped by the mn == mx test above. seq(1, 0, 2) then throws
      # "wrong sign in 'by' argument", which surfaced as a red error where the
      # filters should be.
      n <- length(sliders)
      if (n == 0)
        return(div(style = "color:#888;font-size:12px;font-style:italic;padding-top:4px;",
                   "No range filters for this slate."))
  ...
  # ---------- NFL Preseason ----------
  # Four validation views, in the order you would check them: does the GAME look
  # like a preseason game, does each POSITION GROUP get a sane share, does any
  # DRIVE have nobody on the field (which silently discards its plays), and
  # where does the sim disagree with ETR.
  render_nfl_preseason_visuals <- function(visuals) {
    req(visuals)
    gaps <- if (!is.null(visuals$coverage)) sum(visuals$coverage$Gap) else 0
    fluidRow(column(12,
      box(width = NULL, title = "Preseason Simulation Analysis",
          status = "primary", solidHeader = TRUE,

        div(style = "margin-bottom:14px;color:#bbb;font-size:12px;",
            sprintf("%s games in pool  |  effective sample %s  |  mean margin %+.2f  |  %s sims",
                    format(visuals$pool_size, big.mark = ","),
                    format(visuals$ess, big.mark = ","),
                    visuals$mean_margin, format(visuals$n_sims, big.mark = ","))),

        if (gaps > 0)
          div(style = "margin-bottom:14px;padding:8px 12px;border-left:3px solid #d9534f;background:#2a1a1a;color:#f0c0c0;font-size:12px;",
              sprintf("%d drive(s) have no eligible player at a position. Those plays are discarded — widen the drive windows on the team tabs.", gaps)),

        div(style = "margin-bottom:20px;",
            tags$p(class = "gts-sr-label", "Team Totals"),
            DTOutput("psn_team_table")),
        tags$hr(style = "border-color:#2a2a2a;margin:16px 0;"),

        div(style = "margin-bottom:20px;",
            tags$p(class = "gts-sr-label", "By Position Group"),
            DTOutput("psn_pos_table")),
        tags$hr(style = "border-color:#2a2a2a;margin:16px 0;"),
        ...
        div(tags$p(class = "gts-sr-label", "Who Is On The Field, By Drive"),
            DTOutput("psn_cover_table"))
      )))
  }
  ...
  output$psn_etr_plot <- renderPlotly({
    ...
    lim <- c(0, max(c(d$Sim, d$ETR), na.rm = TRUE) * 1.08)
    plot_ly(d, x = ~ETR, y = ~Sim, type = "scatter", mode = "markers",
            color = ~Pos, text = ~paste0(Player, " (", Team, ")<br>sim ", Sim,
                                         " vs ETR ", ETR),
            hoverinfo = "text", marker = list(size = 9)) %>%
      add_trace(x = lim, y = lim, type = "scatter", mode = "lines",
                inherit = FALSE, showlegend = FALSE,
                line = list(color = "#FFE500", dash = "dash", width = 1)) %>%
